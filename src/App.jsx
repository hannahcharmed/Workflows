...
function WeeklyModelReport({models,sales,campaigns,socialMetrics,content,fans,ttks,users,user}){
  ...
  const handleDownloadPDF=()=>{
    ...
    if(!pw)return;
    pw.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Weekly Report – ${modelName} – ${weekOf}</title><style>
      *{box-sizing:border-box;margin:0;padding:0;}
      body{font-family:Georgia,serif;color:#111;background:#fff;padding:28px 36px;font-size:13px;}
      h1{font-size:22px;font-weight:800;color:#7c3aed;letter-spacing:-0.01em;}
      ...
      .revenue-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px;}
      .rev-box{border:1px solid #e5e7eb;border-radius:8px;padding:12px;text-align:center;}
      .rev-label{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#7c3aed;margin-bottom:4px;}
      .rev-val{font-size:20px;font-weight:800;}
      .section{margin-bottom:14px;padding:12px 16px;border:1px solid #e5e7eb;border-radius:8px;page-break-inside:avoid;}
      .sec-title{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#7c3aed;margin-bottom:7px;}
      .sec-body{font-size:13px;white-space:pre-wrap;line-height:1.7;color:#222;}
      .footer{margin-top:22px;padding-top:10px;border-top:1px solid #e5e7eb;font-size:9px;color:#bbb;text-align:center;}
      .print-btn{position:fixed;top:16px;right:16px;background:#7c3aed;color:#fff;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;font-family:system-ui,sans-serif;box-shadow:0 4px 14px rgba(124,58,237,0.4);}
      .print-btn:hover{background:#6d28d9;}
      @media print{.print-btn{display:none;}@page{margin:12mm;size:A4 portrait;}}
    </style></head><body>
    <button class="print-btn" onclick="window.print()">Save as PDF</button>
    <div class="header">
      <h1>Charmed Collective</h1>
      <div style="font-size:16px;font-weight:700;margin-top:6px;">Weekly Model Report</div>
      <div class="sub">${modelName} &middot; Week of ${weekOf} &ndash; ${weekEnd}</div>
      ${model?`<div class="sub">Account Manager: ${model.am} &middot; Platform: ${model.platform}</div>`:""}
    </div>
    <div class="revenue-grid">
      ${[["Total Revenue",fields.weeklyRevenue],["PPV Revenue",fields.ppvRevenue],["Custom Revenue",fields.customRevenue]].filter(([,v])=>v).map(([l,v])=>`<div class="rev-box"><div class="rev-label">${l}</div><div class="rev-val">${v}</div></div>`).join("")}
    </div>
    ${[["Active Campaigns",fields.activeCampaigns,"📣"],["Top Content This Week",fields.topContent,"📦"],["Social Snapshot",fields.socialSnapshot,"📱"],["Fan Highlights",fields.fanHighlights,"🐋"],["Goals for Next Week",fields.goalsNextWeek,"🎯"],["AM Notes & Instructions",fields.amNotes,"📝"],["Reminders",fields.reminders,"🔔"],["Shoutouts",fields.shoutouts,"🌟"],["Incentives",fields.incentives,"💎"]].filter(([,v])=>v).map(([t,v,e])=>`<div class="section"><div class="sec-title">${e} ${t}</div><div class="sec-body">${v.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</div></div>`).join("")}
    <div class="footer">Generated by Charmed Ops &middot; ${new Date().toLocaleDateString()} &middot; Confidential</div>
    </body></html>`);
    pw.document.close();
  };
    ...
    <div>
      <SectionHeader icon="📄" title="Weekly Model Reports" action={
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <Btn size="sm" onClick={handleDownloadPDF}>Download PDF</Btn>
        </div>
      }/>
